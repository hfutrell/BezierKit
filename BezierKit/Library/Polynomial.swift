    ...
    withUnsafeTemporaryAllocation(of: CGFloat.self, capacity: 6) { buffer in
        ...
        for i in 0..<count - 1 {
            ...
            if fStart * fEnd < 0 {
                ...
                guard Swift.abs(value - guess) < 1.0e-5 else { continue }
                guard Swift.abs(polynomial.value(at: value)) < 1.0e-10 else { continue }
                root = value
            }
            if let last = lastFoundRoot {
                guard last + 1.0e-5 < root else { continue }
            }
            lastFoundRoot = root
            callback(root)
        }
