...
class BezierCurveTests: XCTestCase {
    ...
    func testProject() {
        ...
        let p4 = c.project(point: CGPoint(x: 0.95, y: 1.05)) // should project to p0
        XCTAssertEqual(p4, CGPoint(x: 1.0, y: 1.0))
        let p5 = c.project(point: CGPoint(x: 5.05, y: 1.05)) // should project to p3
        ...
        let p7 = c.project(point: c.compute(0.831211) + c.normal(0.831211), errorThreshold: 1.0e-5) // should project back to (roughly) c.compute(0.831211)
